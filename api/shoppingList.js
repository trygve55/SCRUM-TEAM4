                ...
                if (err) {
                    ...
                    for (var i = 0; i < result.length;i++) {
                        ...
                        for (var j = 0; j < result.length; j++) {
                            if (result[j].shopping_list_entry_id) entries.push({
                                "shopping_list_entry_id":result[j].shopping_list_entry_id,
                                "entry_text":result[j].entry_text,
                                "added_by_person_id":result[j].added_by_person_id,
                                "purchased_by_person_id":result[j].purchased_by_person_id,
                                "cost":result[j].cost,
                                "datetime_added":result[j].datetime_added,
                                "datetime_purchased":result[j].datetime_purchased
                            });
...
function removeDuplicateUsingFilter(arr){
    var added_ids = [], unique_array = arr.filter(function(elem, index, self) {
        if (elem.shopping_list_entry_id) {
